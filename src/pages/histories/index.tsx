...
import { getHistories, deleteHistoryById } from 'apiSdk/histories';
import { HistoryInterface } from 'interfaces/history';

type ColumnType = ColumnDef<HistoryInterface, unknown>;
...
export function HistoryListPage(props: HistoryListPageProps) {
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<HistoryInterface>>(
    () => `/histories?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: HistoryInterface) => {
    if (hasAccess('history', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/histories/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/histories/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('history', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/histories/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('history', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="history" />
        </Flex>

        {hasAccess('history', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/histories/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
