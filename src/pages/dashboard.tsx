...
import { getTours } from 'apiSdk/tours';
...
function DashboardPage() {
  ...
  const tourStat = useSWR<PaginatedInterface<TourInterface>>(
    () => `/tours`,
    () => getTours({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; isLoading: boolean }[] = [
    {
      title: 'Tours',
      result: tourStat?.data?.totalCount,
      isLoading: tourStat.isLoading,
    },
    {
      title: 'Tickets',
